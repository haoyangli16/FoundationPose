...
def make_crop_data_batch(
    ...
    if cfg["use_normal"]:
        ...
        normalBs = kornia.geometry.transform.warp_perspective(
            torch.as_tensor(normal_map, dtype=torch.float, device="cuda")
            .permute(2, 0, 1)[None]
            .expand(B, -1, -1, -1),
            tf_to_crops,
            dsize=render_size,
...
class PoseRefinePredictor:
    ...
    def predict(
        ...
        logging.info(
            f"trans_normalizer:{self.cfg['trans_normalizer']}, rot_normalizer:{self.cfg['rot_normalizer']}"
        )
        bs = 1024
            ...
            for b in range(0, pose_data.rgbAs.shape[0], bs):
                ...
                if self.cfg["trans_rep"] == "tracknet":
                    ...
                    def project_and_transform_to_crop(centers):
                        uvs = (
                            pose_data.Ks[b : b + bs] @ centers.reshape(-1, 3, 1)
                        ).reshape(-1, 3)
                        uvs = uvs / uvs[:, 2:3]
                        uvs = (
                            pose_data.tf_to_crops[b : b + bs] @ uvs.reshape(-1, 3, 1)
                        ).reshape(-1, 3)
                        return uvs[:, :2]

                    rot_delta = output["rot"]
                    z_pred = (
                        output["trans"][:, 2] * pose_data.poseA[b : b + bs][..., 2, 3]
                    )
                    uvA_crop = project_and_transform_to_crop(
                        pose_data.poseA[b : b + bs][..., :3, 3]
                    )
                    uv_pred_crop = (
                        uvA_crop + output["trans"][:, :2] * self.cfg["input_resize"][0]
                    )
                    uv_pred = transform_pts(
                        uv_pred_crop, pose_data.tf_to_crops[b : b + bs].inverse().cuda()
                    )
                    center_pred = torch.cat(
                        [
                            uv_pred,
                            torch.ones(
                                (len(rot_delta), 1), dtype=torch.float, device="cuda"
                            ),
                        ],
                        dim=-1,
                    )
                    center_pred = (
                        pose_data.Ks[b : b + bs].inverse().cuda()
                ...
                if self.cfg["rot_rep"] == "axis_angle":
                    ...
                    rot_mat_delta = rotation_6d_to_matrix(output["rot"]).permute(
                        0, 2, 1
                    )
                else:
                    raise RuntimeError
